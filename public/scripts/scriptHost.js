...
const container_notification_msg = document.getElementById(
  "container-notifcationmsg"
);
...
//Millicast straming html elements
const startStream = document.getElementById("stream-btn");
const spanStream = document.getElementById("span-stream");
const iStream = document.getElementById("i-stream");

const millicastPlayer = document.getElementById("open-millicast");

//Controls buttons to start/stop & publish Millicast Stream
...
const handleStartStopStream = async () => {
  conferenceID = VoxeetSDK.conference.current.id;
  console.log(conferenceID);
  response = await fetch(`/apiAccessToken`);
  jsonResponse = await response.json();
  accessToken = jsonResponse.accessToken;
  console.log("API", accessToken);

  response = await fetch(`/millicastValues`);
  jsonResponse = await response.json();
  const millicast_stream_name = jsonResponse.millicast_stream_name;
  const millicast_publishing_token = jsonResponse.millicast_publishing_token;

  if (stopStream === false) {
    try {
      console.log("handleStartStopStream");
      ...
      const options = {
        ...
        body: JSON.stringify({
          streamName: millicast_stream_name,
          publishingToken: millicast_publishing_token,
        }),
      };
      console.log(options);
      let url = `https://comms.api.dolby.io/v2/conferences/mix/${conferenceID}/lls/start`;
      const response = await fetch(url, options);
      console.log("lls start response", response.status);
      if (response.status == 200) {
        spanStream.style.color = "red";
        iStream.classList.remove("fa-play");
        iStream.classList.add("fa-circle-stop");
        millicastPlayer.style.display = "initial";
        stopStream = true;
      } else {
        throw "LLS Start Unsuccessfull";
      }
    } catch (error) {
      console.log("handleStartStopStream: ", error);
    }
  } else {
    try {
      const options = {
      ...
      let url = `https://comms.api.dolby.io/v2/conferences/mix/${conferenceID}/lls/stop`;
      console.log(options);
      const response = await fetch(url, options);
      if (response.status === 200) {
        spanStream.style.color = "green";
        iStream.classList.remove("fa-circle-stop");
        iStream.classList.add("fa-play");
        millicastPlayer.style.display = "none";
        stopStream = false;
      } else {
        throw "LLS Stop Unsuccessfull";
      }
    } catch (error) {
      console.log("stop stream: True, handleStartStopStream: ", error);
    }
  }
};
...
const InviteParticipanttotheMeeting = async (participantName) => {
  ...
  if (conferenceAliasInput === "") {
    // console.log("alias", conferenceAliasInput);
    conferenceAliasInput = `${externalID} and ${participantUser}'s meeting`;
    CreateandJoinConference(conferenceAliasInput);
    container_notification_msg.style.display = "initial";
  }
  await sleep(1000);
  ...
  const btnGroup = document.getElementById("btn-group-" + participantUser);

  btnGroup.innerHTML = "";

  const switchUserBtn = document.createElement("button");
  switchUserBtn.setAttribute("type", "button");
  switchUserBtn.setAttribute("class", "btn btn-secondary m-1 ");
  switchUserBtn.setAttribute("name", participantUser);
  switchUserBtn.setAttribute("onclick", "switchfromListenertoUser(this)");
  switchUserBtn.innerHTML = "Promote to speaker";

  btnGroup.appendChild(switchUserBtn);
  // htmlParentElement.remove();
  console.log(htmlParentElement);
};

const switchfromListenertoUser = async (participantName) => {
...
const JoinConference = async () => {
  try {
    ...
    const confObject = await VoxeetSDK.conference.fetch(conferenceID);
    const joinOptions = {
      constraints: {
        audio: false,
        video: false,
...
listener = {
  ...
  message(response) {
    if (externalID === response.userMetadata.uuid) {
      ...
      notification_msg.innerText = msg;
    }
  },
  presence(response) {
