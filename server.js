...
millicast_stream_name = process.env.MILLICAST_STREAM_NAME;
millicast_publishing_token = process.env.MILLICAST_PUBLISHING_TOKEN;
...
app.get("/apiAccessToken", async function (request, response) {
  const APP_KEY = client_key;
  const APP_SECRET = client_secret;
...
app.get("/millicastValues", async function (request, response) {
  response.send({
    millicast_stream_name: millicast_stream_name,
    millicast_publishing_token: millicast_publishing_token,
  });
});
